...
public class LogReport {
	private  final static String TAG = "LogReport";
	...
	private static int sFileSizeLimit; // In bytes
	private static int isRelease = 1;



	public static void open(String logFilePath, int currentPriority,
                            int fileSizeLimit) {
		...
		try {
			sBufferedWriter = new BufferedWriter(new FileWriter(sTheLogFile,
					true));
		} catch (IOException e) {
			Log.e(TAG, Log.getStackTraceString(e));
		}
	...
	private static void writeToFile(int priority, String tag, String msg,
		...
		if ((priority >= sCurrentPriority) && (sBufferedWriter != null)) {
			try {
				if (checkFileSize()) {
					sBufferedWriter = new BufferedWriter(new FileWriter(
							sTheLogFile, true));
				}
	...
	private static String getCurrentTimeStamp() {
		...
		try {
			SimpleDateFormat dateFormat = new SimpleDateFormat(
					TIMESTAMP_FORMAT, java.util.Locale.getDefault());
